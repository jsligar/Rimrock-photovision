...
function renderClusterList() {
  ...
  const tagged = clusters.filter(c => c.person_label && !c.is_noise);
  ...
  taggedGroups.forEach((group, key) => {
  ...
  const untaggedGroups = [
    { key: 'pending', name: 'Pending', items: untagged.filter(c => !c.is_noise) },
    { key: 'noise', name: 'Noise', items: untagged.filter(c => c.is_noise) },
  ];
