...
def _get_clip():
    ...
    if _clip_model is None:
        ...
        stored_model = db.pipeline_meta_get("clip_model")
        if stored_model and stored_model != config.CLIP_MODEL:
            log.warning(
                "CLIP model mismatch: embeddings were generated with '%s' but "
                "the current config uses '%s'. Semantic search results may be "
                "inaccurate. Re-run Process phase to regenerate embeddings.",
                stored_model,
                config.CLIP_MODEL,
            )
...
def _burst_enhanced_rank(query: str, conn, top_k: int) -> tuple[list[tuple[int, float]], dict]:
    """Hybrid rank with optional NVIDIA burst query rewrite + reranking.

    Returns (ranked_results, burst_meta) where burst_meta describes what burst did.
    Only called when NVIDIA_BURST_ENABLED is True.
    """
    burst_meta: dict = {"query_rewrite": None, "reranked": False}

    # Step 1: Query rewrite — expand the query via LLM
    effective_query = query
    expanded = nvidia_burst.query_rewrite(query)
    if expanded:
        burst_meta["query_rewrite"] = expanded
        effective_query = expanded

    # Step 2: Local hybrid rank with the (possibly expanded) query
    local_results = _hybrid_rank(effective_query, conn, top_k * 2)

    # Step 3: Rerank the top candidates via hosted reranker
    if not local_results:
        return local_results, burst_meta

    rerank_candidates = local_results[: config.NVIDIA_BURST_RERANK_TOP_N]
    candidate_ids = [pid for pid, _ in rerank_candidates]

    # Build text representations for the reranker
    photo_texts = _build_rerank_texts(conn, candidate_ids)
    if photo_texts:
        docs = [{"text": photo_texts.get(pid, ""), "photo_id": pid} for pid in candidate_ids]
        reranked = nvidia_burst.rerank(query, docs, top_n=top_k)
        if reranked:
            burst_meta["reranked"] = True
            # Merge: reranked results first, then remaining local results
            reranked_ids = {d["photo_id"] for d in reranked}
            reranked_scored = [(d["photo_id"], d["rerank_score"]) for d in reranked]
            # Append non-reranked tail from local results
            tail = [(pid, score) for pid, score in local_results if pid not in reranked_ids]
            return (reranked_scored + tail)[:top_k], burst_meta

    return local_results[:top_k], burst_meta


def _build_rerank_texts(conn, photo_ids: list[int]) -> dict[int, str]:
    """Build text summaries for photos to feed into the reranker."""
    if not photo_ids:
        return {}
    placeholders = ",".join("?" * len(photo_ids))

    rows = conn.execute(
        f"""SELECT photo_id, filename, existing_people, ocr_text
            FROM photos WHERE photo_id IN ({placeholders})""",
        photo_ids,
    ).fetchall()

    tag_rows = conn.execute(
        f"""SELECT photo_id, tag FROM photo_tags
            WHERE photo_id IN ({placeholders})""",
        photo_ids,
    ).fetchall()
    ...
    for row in tag_rows:
        tags_by_photo.setdefault(int(row["photo_id"]), []).append(row["tag"])

    texts: dict[int, str] = {}
    for row in rows:
        pid = int(row["photo_id"])
        parts = [row["filename"] or ""]
        if row["existing_people"]:
            parts.append(f"people: {row['existing_people']}")
        tags = tags_by_photo.get(pid, [])
        if tags:
            parts.append(f"tags: {', '.join(tags)}")
        if row["ocr_text"]:
            parts.append(f"text: {row['ocr_text'][:200]}")
        texts[pid] = " | ".join(p for p in parts if p)
...
def semantic_search(
    ...
    try:
        ...
        elif mode == "hybrid" and nvidia_burst.is_enabled():
            ranked, burst_meta = _burst_enhanced_rank(q, conn, top_k)
        elif mode == "hybrid":
