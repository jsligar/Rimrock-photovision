...
def _approved_source_centroids(conn, exclude_cluster_id: int) -> list[dict]:
    rows = conn.execute(
        """SELECT f.cluster_id, f.embedding, c.person_label
           FROM faces f
...
def cluster_suggestions(cluster_id: int, top_k: int = 3):
    ...
    sources = _approved_source_centroids(conn, exclude_cluster_id=cluster_id)
    if not sources:
        conn.close()
    ...
    top = scored[:max(1, min(int(top_k), 10))]
    ...
    for i, item in enumerate(top):
        ...
        item["recommended"] = bool(i == 0 and item["score"] >= 0.45 and item["margin"] >= 0.03)
