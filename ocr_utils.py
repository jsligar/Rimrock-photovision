...
def load_local_image_rgb(photo_path: Path, *, draft_max_dim: int | None = None) -> Image.Image:
    """Load a trusted local image as RGB, allowing oversized phone captures when needed."""
    target_dim = max(64, int(draft_max_dim or config.SEARCH_OCR_MAX_DIM))
    ...
    with _allow_large_local_images():
        with Image.open(photo_path) as img:
            # Ask Pillow for a lower-resolution decode when possible before we convert.
            try:
