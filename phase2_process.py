...
import config
import db
from ocr_utils import extract_ocr_text, load_local_image_rgb, tesseract_available
...
def _process_single_photo(
    ...
    try:
        ...
        # Persist the image embedding for similarity search.
        # Serialization: raw float32 bytes — matches np.frombuffer(..., dtype=np.float32)
        # used in api/routes/search.py _blob_to_vec().
    ...
    # ── OCR Pass ──
    if config.SEARCH_OCR_ENABLED and tesseract_available():
        try:
            ocr_pil = load_local_image_rgb(photo_path, draft_max_dim=config.SEARCH_OCR_MAX_DIM)
            ocr_text = extract_ocr_text(ocr_pil)
            del ocr_pil  # free before DB write to keep unified memory low
            conn.execute(
                "UPDATE photos SET ocr_text=?, ocr_extracted_at=? WHERE photo_id=?",
                (ocr_text, _now(), photo_id),
            )
            conn.commit()
        except Exception as e:
            log.warning("OCR failed for %s: %s", rel_path, e)

    # ── Mark processed (only if CLIP succeeded) ──
    if clip_ok:
        conn.execute(
            "UPDATE photos SET processed_at=?, process_error=NULL WHERE photo_id=?",
            (_now(), photo_id),
        )
    else:
        conn.execute(
            "UPDATE photos SET process_error=? WHERE photo_id=?",
            ("CLIP embedding failed — will retry", photo_id),
        )
    conn.commit()
    conn.close()
    return clip_ok
